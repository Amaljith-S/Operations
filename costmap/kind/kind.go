...
func KindResourceUsageFinder(kindNameForUsage string, resourceType string, userCostInput InputCostData, fromDate string, toDate string, timeinHours float64) (meadianValue float64) {
	...
	if err := json.NewDecoder(res3.Body).Decode(&mapResp); err != nil {
		...
		if resourceType == "Cpu" {
			...
			fmt.Println(resourceType, "Cost of", kindNameForUsage, "is : $", ((median*userCostInput.CPUCost)*timeinHours)/1000)
			return median
		...
		if resourceType == "Memory" {
			...
			fmt.Println(resourceType, "Cost of", kindNameForUsage, "is : $", ((median*userCostInput.MemoryCost)*timeinHours)*0.001048576)
			return median
