	...
	for _, pod := range pods.Items {
		...
		if len(pod.OwnerReferences) == 0 {
			...
			for _, container1 := range podContainers1 {
				...
				// fmt.Println("@@@@@@@@@@@@@@@@@@@@@@@@@@",stringCpuWithOutOwner)
		...
		default:
			fmt.Printf("Could not find resource manager fon", pod.OwnerReferences[0].Kind)
			continue
		}
		...
		var cpuWithOwner int64    
		var memoryWithOwner int64 
		...
		for _, container := range podContainers {
			...
			// fmt.Println(podName,"###########",memQuantity)
